...
class SpeechScoringSystem {
    constructor() {
        ...
        this.pollingFrequency = 5000; // 轮询频率（毫秒），默认5秒
    ...
    async handleUrlChange() {
        ...
        if (newSessionId && newSessionId !== this.sessionId) {
            console.log(`检测到URL变化，新的会话ID: ${newSessionId}`);
            ...
            if (loaded) {
                ...
                this.showToast('这是一个新会话，暂无数据', 'info');
    ...
    startIncrementalPolling(forceRestart = false) {
        console.log('[启动轮询] 开始启动增量轮询...');
        console.log('[启动轮询] 当前轮询状态:', this.isPolling);
        console.log('[启动轮询] 强制重启:', forceRestart);

        if (this.isPolling && !forceRestart) {
            console.log('[启动轮询] 轮询已在运行中，跳过启动');
            return;
        }

        // 如果是强制重启，先停止现有轮询
        if (forceRestart && this.isPolling) {
            console.log('[启动轮询] 强制重启：先停止现有轮询');
            this.stopIncrementalPolling();
        }

        console.log('[启动轮询] 正在初始化LeanCloud...');
        const lcInitialized = this.initLeanCloud();
        console.log('[启动轮询] LeanCloud初始化结果:', lcInitialized);

        if (!lcInitialized) {
            console.error('[启动轮询] ❌ LeanCloud未初始化，无法启动轮询');
            console.error('[启动轮询] 请检查LeanCloud配置和SDK加载');
            return;
        }

        this.isPolling = true;
        console.log('[启动轮询] ✅ 轮询状态已设置为 true');
        console.log('[启动轮询] 轮询频率:', this.pollingFrequency, 'ms');

        // 立即执行一次同步
        console.log('[启动轮询] 立即执行第一次同步...');
        this.fetchIncrementalUpdates();

        // 设置定时轮询
        console.log('[启动轮询] 设置定时轮询...');
        this.pollingInterval = setInterval(() => {
            this.fetchIncrementalUpdates();
        }, this.pollingFrequency);

        console.log('[启动轮询] ✅ 增量轮询启动成功！');
    }
    ...
    async fetchIncrementalUpdates() {
        try {
            ...
            console.log(`[增量轮询] 查询会话 ${this.sessionId} 在 ${new Date(this.lastSyncTimestamp).toLocaleString()} 之后的更新`);

            // 查询该会话在lastSyncTimestamp之后的所有更新
            ...
            query.limit(100); // 限制每次获取的数量

            const results = await query.find();

            if (results.length > 0) {
                console.log(`[增量轮询] 获取到 ${results.length} 条增量更新`);

                // 处理每条更新
                for (const result of results) {
                    const dataType = result.get('dataType');
                    console.log(`[增量轮询] 处理更新: ${dataType}, objectId: ${result.id}`);
                ...
                console.log(`[增量轮询] 更新时间戳: ${new Date(this.lastSyncTimestamp).toLocaleString()}`);
                ...
                if (adminSection && adminSection.style.display !== 'none') {
                    console.log('[增量轮询] 检测到管理员界面打开，自动刷新界面');
                    this.updateAdminInterface();
                }

                // 如果当前在评委界面，更新评委界面
                if (judgeSection && judgeSection.style.display !== 'none') {
                    console.log('[增量轮询] 检测到评委界面打开，自动刷新界面');
                    this.updateJudgeInterface();
                }

                // 如果当前在结果页面，更新结果显示
                if (resultsSection && resultsSection.style.display !== 'none') {
                    console.log('[增量轮询] 检测到结果页面打开，自动刷新结果');
    ...
    mergeUsers(newUsers) {
        ...
        if (this.currentClass && this.currentClass.isFixed) {
            console.log('固定班级不合并用户数据，保持当前数据');
            return;
        }
        ...
        console.log('用户数据已合并，当前用户数:', this.users.length);
    }
    ...
    async saveToCloud(dataType, data) {
        try {
            ...
            console.log(`数据已保存到云端: ${dataType}, objectId: ${sessionData.id}`);

            // 保存成功后，立即更新当前界面（如果是管理员界面）
            const adminSection = document.getElementById('adminSection');
            if (adminSection && adminSection.style.display !== 'none') {
                console.log('[保存后更新] 检测到管理员界面打开，立即刷新界面');
                // 使用 setTimeout 确保在当前调用栈完成后执行
                setTimeout(() => {
                    this.updateAdminInterface();
                }, 100);
            }
    ...
    async loadFromCloud() {
        try {
            ...
            if (!this.sessionId) {
                console.warn('会话ID未设置，无法从云端加载');
                return false;
            }

            console.log('从云端加载初始数据...');
            ...
            query.limit(50); // 获取最近的50条记录

            const results = await query.find();

            if (results.length > 0) {
                console.log(`从云端获取到 ${results.length} 条数据`);
                ...
                // 设置最后同步时间戳为最新记录的时间（减去1秒，以确保能获取到边界数据）
                const latestUpdate = results[0].updatedAt.getTime();
                this.lastSyncTimestamp = latestUpdate - 1000;  // 减去1秒
                localStorage.setItem(`lastSyncTimestamp_${this.sessionId}`, this.lastSyncTimestamp.toString());

                this.showNetworkSuccess('云端数据加载成功');
                return true;
            } else {
                console.log('云端没有该会话的数据');
    ...
    updateScoringProgress() {
        ...
        if (this.scoringStarted && this.speakers.length > 0 && this.judges.length > 0) {
            ...
            if (progressArea) {
                ...
                progressHTML += `<th>完成度</th></tr></thead><tbody>`;
                ...
                this.speakers.forEach(speaker => {
                    ...
                    this.judges.forEach(judge => {
                        ...
                        if (hasScore) {
                            ...
                            progressHTML += `<td class="scored">✅ ${score}分</td>`;
                        } else {
                            progressHTML += `<td class="not-scored">⏳ 未评分</td>`;
                        }
                    });
    ...
    async switchToClass(classId) {
        try {
            ...
            if (selectedClass.isFixed && selectedClass.students && selectedClass.students.length > 0) {
                console.log(`正在加载固定班级 ${selectedClass.name} 的学生名单...`);
                ...
                console.log(`已更新总人数限制为: ${this.settings.maxUsers}`);
    ...
    initializeFixedClasses() {
        // 从 fixed_classes_data.js 获取学生名单
    ...
    adminLogin(username, password) {
        ...
        try {
            this.updateClassManagementInterface();
            console.log('班级管理界面已更新，当前班级数量:', this.classes.length);
        } catch (e) {
            console.error('更新班级管理界面失败:', e);
        }
        try { this.updateAdminInterface(); } catch (e) { console.error('更新管理员界面失败:', e); }
    ...
    // 云端订阅相关方法
    // 旧的 LiveQuery 订阅方法已移除，现在使用时间戳增量轮询
    async startCloudSubscription() {
        console.log('========================================');
        console.log('[云端订阅] 启动云端订阅：使用时间戳增量轮询');
        console.log('[云端订阅] 当前用户:', this.currentUser);
        console.log('[云端订阅] 会话ID:', this.sessionId);
        console.log('========================================');

        // 先尝试从云端加载最新数据
        console.log('[云端订阅] 步骤1：从云端加载历史数据...');
        try {
            const loaded = await this.loadFromCloud();
            console.log('[云端订阅] 云端数据加载结果:', loaded);
        } catch (error) {
            console.error('[云端订阅] 从云端加载数据失败:', error);
        }

        // 启动增量轮询
        console.log('[云端订阅] 步骤2：启动增量轮询...');
        this.startIncrementalPolling();

        console.log('[云端订阅] 最终轮询状态:', this.isPolling);
        console.log('========================================');
    }

    stopCloudSubscription() {
        // 停止增量轮询
        console.log('停止云端订阅：停止时间戳增量轮询');
        this.stopIncrementalPolling();
    }
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        window.addEventListener('online', () => {
            console.log('网络已连接，启动增量轮询');
            system.startIncrementalPolling();
        });

        window.addEventListener('offline', () => {
            console.log('网络已断开，停止增量轮询');
        ...
        if (enableSyncCheckbox) {
            enableSyncCheckbox.addEventListener('change', (e) => {
                if (e.target.checked) {
                    ...
                    if (statusDisplay) statusDisplay.textContent = '已启用';
                } else {
                    system.stopIncrementalPolling();
                    const statusDisplay = document.getElementById('syncStatusDisplay');
                    if (statusDisplay) statusDisplay.textContent = '已禁用';
                }
            });
        }
